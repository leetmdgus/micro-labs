...
# Utils
# -----------------------------------------------------------------------------
def _remove_linesegarray_in_paragraph(p_el: etree._Element) -> bool:
    """
    해당 hp:p 내부의 hp:linesegarray 를 제거해서 한글이 레이아웃을 재계산하게 함.
    """
    changed = False
    for lsa in p_el.xpath(".//hp:linesegarray", namespaces=NS):
...
def _collect_paragraphs_between(fb: etree._Element, end_node: etree._Element) -> List[etree._Element]:
    """
    fieldBegin ~ fieldEnd 사이에 등장하는 hp:p(문단)들을 수집.
    """
    ps: List[etree._Element] = []
    # fb의 다음 노드부터 end_node 직전까지 document-order로 훑는다
    for el in fb.itersiblings():
        # itersiblings()는 같은 부모 아래만 보므로, field 구조가 복잡할 때를 대비해
        # xpath로 end까지 범위 지정하는 방식이 더 낫다.
        break

    # XPath로 'fb 이후 end_node 이전'에 있는 p를 수집 (end_node 기준으로 1개 구간)
    # fb 다음에 등장하는 p 중에서, end_node 이전에 있는 p만
    ps = fb.xpath(
        "following::hp:p[following::hp:fieldEnd[@beginIDRef=$bid][1] = $end]",
        namespaces=NS,
        bid=fb.get("id"),
        end=end_node,
    )
...
def _set_xml_space_preserve(t_el: etree._Element) -> None:
...
# -----------------------------------------------------------------------------
# (C) 텍스트 주입: fieldBegin~fieldEnd 범위의 내용을 "run 내부 구조"로 교체
#     - 이 템플릿은 줄바꿈을 hp:lineBreak 로 표현함 (사용자 제공 XML 근거)
# -----------------------------------------------------------------------------
def _replace_run_content_between_begin_end(
    begin_ctrl: etree._Element,
    end_ctrl: etree._Element,
    value: str,
) -> bool:
    """
    begin_ctrl, end_ctrl 는 같은 hp:run 안에 있어야 함.
    begin_ctrl 다음부터 end_ctrl 직전까지의 hp:t / hp:lineBreak 를 제거하고,
    value를 hp:t + hp:lineBreak 구조로 삽입한다.
    """
    run = begin_ctrl.getparent()
    if run is None:
        return False
    if run != end_ctrl.getparent():
        return False

    children = list(run)
    try:
        i_begin = children.index(begin_ctrl)
        i_end = children.index(end_ctrl)
    except ValueError:
        return False
    ...
    # (1) begin~end 사이에 있는 hp:t / hp:lineBreak만 제거 (다른 태그는 보존)
    changed = False
    between = children[i_begin + 1 : i_end]
    for node in between:
        if node.tag in (f"{{{HP_NS}}}t", f"{{{HP_NS}}}lineBreak"):
            run.remove(node)
            changed = True
    ...
    insert_pos = i_begin + 1  # begin_ctrl 바로 뒤에 삽입
    for idx, line in enumerate(lines):
        t = etree.Element(f"{{{HP_NS}}}t")
        _set_xml_space_preserve(t)
        t.text = line
        run.insert(insert_pos, t)
        insert_pos += 1
        changed = True

        # 마지막 줄이 아니면 lineBreak 삽입
        if idx != len(lines) - 1:
            lb = etree.Element(f"{{{HP_NS}}}lineBreak")
            run.insert(insert_pos, lb)
            insert_pos += 1
            changed = True

    return changed

def _remove_linesegarray_in_scope(fb: etree._Element, end_node: etree._Element) -> bool:
    """
    fieldBegin~fieldEnd 범위에 포함되는 hp:linesegarray를 제거해서
    한글이 열 때 레이아웃을 재계산하도록 유도.
    """
    changed = False
    lsa_nodes = fb.xpath(
        "following::hp:linesegarray[following::hp:fieldEnd[@beginIDRef=$bid][1] = $end]",
        namespaces=NS,
        bid=fb.get("id"),
        end=end_node,
    )
    for lsa in lsa_nodes:
        parent = lsa.getparent()
        if parent is not None:
            parent.remove(lsa)
            changed = True
...
def _remove_existing_text_and_breaks_in_scope(fb: etree._Element, end_node: etree._Element) -> bool:
    """
    범위 내 hp:t 텍스트와 hp:lineBreak를 비워/제거.
    (기존 텍스트 흔적이 남아서 겹치는 것 방지)
    """
    changed = False

    # lineBreak 제거
    lb_nodes = fb.xpath(
        "following::hp:lineBreak[following::hp:fieldEnd[@beginIDRef=$bid][1] = $end]",
        namespaces=NS,
        bid=fb.get("id"),
        end=end_node,
    )
...
def _inject_multiline_into_first_run(fb: etree._Element, end_node: etree._Element, value: str) -> bool:
    """
    범위 내 첫 번째 hp:t 위치(대개 begin run 내부)를 기준으로:
    첫 줄은 hp:t에 넣고, 이후 줄은 hp:lineBreak + hp:t를 같은 run에 추가.
    """
    # first hp:t in scope
    t_nodes = fb.xpath(
        "following::hp:t[following::hp:fieldEnd[@beginIDRef=$bid][1] = $end]",
    ...
    if run is None or run.tag != f"{{{HP_NS}}}run":
        # 예상 구조가 아니면 포기
        return False

    # 기존 scope 텍스트/줄바꿈 정리
    changed = _remove_existing_text_and_breaks_in_scope(fb, end_node)
    ...
    # 첫 줄은 first_t에
    _set_xml_space_preserve(first_t)
    if (first_t.text or "") != lines[0]:
        first_t.text = lines[0]
        changed = True

    # 나머지는 run 안에 연속 삽입 (first_t 바로 뒤)
    insert_pos = run.index(first_t) + 1
    for line in lines[1:]:
        lb = etree.Element(f"{{{HP_NS}}}lineBreak")
        run.insert(insert_pos, lb)
        insert_pos += 1
...
def fill_fields_in_xml(xml_bytes: bytes, values: Dict[str, str]) -> Tuple[bytes, bool]:
    ...
    for fb in root.xpath(".//hp:fieldBegin[@name][@id]", namespaces=NS):
        ...
        target_value = str(values[slot_name])
        ...
        # fb가 들어있는 begin_ctrl / end_ctrl 찾기
        begin_ctrl = fb.getparent()
        if begin_ctrl is None or begin_ctrl.tag != f"{{{HP_NS}}}ctrl":
            begin_ctrl = None

        end_ctrl = end_node.getparent()
        if end_ctrl is None or end_ctrl.tag != f"{{{HP_NS}}}ctrl":
            end_ctrl = None

        # (1) run 기반 치환 루틴이 있으면 우선 시도
        replaced_by_run = False
        if begin_ctrl is not None and end_ctrl is not None:
            if _replace_run_content_between_begin_end(begin_ctrl, end_ctrl, target_value):
                replaced_by_run = True
                changed = True

        # (2) 줄바꿈 안정 주입: (1)이 실패했거나, (1)이 성공했더라도 텍스트가 겹치는 문제 방지용으로
        #     - 템플릿이 care_number처럼 hp:lineBreak 구조를 쓰는 경우, 이 방식이 가장 잘 맞음
        #     - 단, (1)이 이미 완벽히 처리하고 있다면 중복 삽입을 피해야 하므로, (1) 성공 시에는 스킵
        if not replaced_by_run:
            if _inject_multiline_into_first_run(fb, end_node, target_value):
                changed = True
            else:
                # 정말 마지막 fallback: 기존처럼 첫 t에만 넣기(줄바꿈은 표시 안 될 수 있음)
                t_nodes = fb.xpath(
                    "following::hp:t[following::hp:fieldEnd[@beginIDRef=$bid][1] = $end]",
                    namespaces=NS,
                    bid=begin_id,
                ...
                if t_nodes:
                    _set_xml_space_preserve(t_nodes[0])
                    if (t_nodes[0].text or "") != target_value:
                        t_nodes[0].text = target_value
                        changed = True
                    for extra in t_nodes[1:]:
                        _set_xml_space_preserve(extra)
                        if extra.text:
                            extra.text = ""
                            changed = True

        # (3) 가장 중요: 겹침/줄배치 꼬임 방지 위해 해당 슬롯 범위의 linesegarray 제거
        if _remove_linesegarray_in_scope(fb, end_node):
            changed = True
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            if "care_number" in payload:
                s = payload["care_number"]
                print("[DBG] care_number len=", len(s), "newline_count=", s.count("\n"))
                print("[DBG] care_number repr=", repr(s[:200]))
