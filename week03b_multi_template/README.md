# week03b_multi_template

**목표: 문서 자동화 멀티 템플릿 환경에서 대응하기**
...
# main.py
"""
HWPX Slot Filler (Local) - Multi Template + slot_map.json files (Safe Edition)

기능:
- templates/ 폴더에 여러 .hwpx 템플릿 지원
- 템플릿 선택 -> 해당 템플릿 slot_map.json 로드 -> 슬롯 입력 UI 렌더
- final.hwpx 생성(선택된 템플릿 기반)
- 원본 템플릿 다운로드
- slot_map은 generated_slot_maps/<template_id>.slot_map.json 으로 파일 생성
  - 템플릿 수정시간이 slot_map보다 최신이면 자동 재생성

텍스트 슬롯:
- hp:fieldBegin(name=slot) ~ hp:fieldEnd(beginIDRef=fieldBegin.id) 범위 안의 hp:t 텍스트 치환

이미지 슬롯(IMG_ 접두사):
- 해당 슬롯의 fieldEnd "뒤"에서 가장 가까운 hp:pic을 찾고
- pic 내부 hc:img@binaryItemIDRef가 가리키는 BinData 파일을 교체

중요:
- 한글이 HWPX 구조에 매우 민감하므로 XML 재직렬화를 최소화
- 파싱 실패 시 원본 유지
- (기본값) CLICK_HERE 제거는 OFF (열림 문제 방지)
...
# Config
# -----------------------------------------------------------------------------
TEMPLATES_DIR = "templates"               # 여러 템플릿 .hwpx 저장
SLOT_MAP_DIR = "generated_slot_maps"     # 템플릿별 slot_map.json 생성 폴더
...
# slot_map 자동 갱신: 템플릿 파일이 더 최신이면 재생성
AUTO_REBUILD_SLOT_MAP = True
...
def list_templates() -> List[Dict[str, str]]:
    ...
    for p in sorted(base.glob("*.hwpx")):
...
def resolve_template_path(template_id: str) -> Path:
    """
    template_id로 templates 폴더 내 hwpx를 안전하게 찾음
    """
    base = Path(TEMPLATES_DIR).resolve()
    target = (base / f"{template_id}.hwpx").resolve()
...
def slot_map_path_for(template_id: str) -> Path:
    Path(SLOT_MAP_DIR).mkdir(parents=True, exist_ok=True)
    return Path(SLOT_MAP_DIR) / f"{template_id}.slot_map.json"


def ensure_slot_map_file(template_id: str) -> Path:
    """
    templates/<id>.hwpx 를 읽어서 generated_slot_maps/<id>.slot_map.json 생성/갱신
    - AUTO_REBUILD_SLOT_MAP=True 이면 템플릿 mtime이 더 최신일 때 재생성
    """
    tpl_path = resolve_template_path(template_id)
    out = slot_map_path_for(template_id)

    if out.exists() and AUTO_REBUILD_SLOT_MAP:
        try:
            tpl_mtime = tpl_path.stat().st_mtime
            out_mtime = out.stat().st_mtime
            if out_mtime >= tpl_mtime:
                return out
        except Exception:
            # stat 실패 시 재생성 시도
            pass

    if out.exists() and not AUTO_REBUILD_SLOT_MAP:
        return out

    slot_map = build_slot_map_from_hwpx(str(tpl_path), only_contents=True)
    out.write_text(json.dumps(slot_map, ensure_ascii=False, indent=2), encoding="utf-8")
    return out


# -----------------------------------------------------------------------------
# (B) index.html 생성 (템플릿 선택 + slot_map.json fetch)
# -----------------------------------------------------------------------------
def ensure_assets() -> None:
    # f-string을 쓰지 않고 placeholder 치환만 사용 (JS의 { } 충돌 방지)
    html = """<!doctype html>
<html lang="ko">
<head>
  <meta charset="utf-8" />
...
<div class="wrap">
  ...
  <p>
    ...
    이미지 슬롯(<span class="mono">__IMG_PREFIX__</span>로 시작)은 파일 업로드로 처리됩니다.
    <span class="small">(STRIP_CLICK_HERE=__STRIP_CLICK_HERE__)</span>
  </p>
...
<script>
  ...
  function renderSlots(slotMap) {
    ...
    slotNames.forEach(name => {
      if (name.startsWith('__IMG_PREFIX__')) {
...
"""
    html = html.replace("__IMG_PREFIX__", IMG_PREFIX)
    html = html.replace("__STRIP_CLICK_HERE__", str(STRIP_CLICK_HERE).lower())
...
# (G) HWPX 생성: 최소 변경 복사
# -----------------------------------------------------------------------------
def generate_submit_hwpx(input_hwpx: str, output_hwpx: str, payload_values: Dict[str, Any], only_contents: bool = True) -> None:
    # 분리
    text_values: Dict[str, str] = {}
    image_values: Dict[str, Dict[str, str]] = {}

    for k, v in (payload_values or {}).items():
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if p == "/slot_map.json":
            qs = parse_qs(q)
            template_id = (qs.get("template", [""])[0] or "").strip()
            if not template_id:
                return self._send(400, b"template required")

            try:
                fp = ensure_slot_map_file(template_id)
                return self._send(200, fp.read_bytes(), "application/json; charset=utf-8")
            except Exception as e:
                return self._send(500, f"Error: {e}".encode("utf-8"))

        if p == "/download_template":
            qs = parse_qs(q)
            template_id = (qs.get("template", [""])[0] or "").strip()
            if not template_id:
                return self._send(400, b"template required")
            ...
            self.send_header("Content-Disposition", f'attachment; filename="{template_id}.hwpx"')
    ...
    def do_POST(self):
        ...
        try:
            ...
            if not isinstance(values, dict):
                raise ValueError("Invalid values: must be object")

            tpl_path = resolve_template_path(template_id.strip())

            out_path = "final.hwpx"
            generate_submit_hwpx(str(tpl_path), out_path, values, only_contents=True)
...
def main():
    ...
    # 서버 시작 시 slot_map.json 파일을 템플릿별로 미리 생성/갱신
    for t in list_templates():
        tid = t.get("id")
        if not tid:
            continue
        try:
            ensure_slot_map_file(tid)
        except Exception as e:
            print(f"[WARN] slot_map 생성 실패: {tid} -> {e}")
...
![image-20260118220458955](..\images\image-20260118220458955.png)

![image-20260118221603557](..\images\image-20260118221603557.png)
