# Week02c_populate_HWPX_slots with_data

**목표: HWPX 내부 슬롯에 데이터를 치환하기**
...
week02a의 방법으로 내부 데이터를 확인한다. 

<table>
  <tr>
    <td><img src="../images/image-20260116205845394.png" width="420"></td>
    <td><img src="../images/image-20260116205929945.png" width="420"></td>
  </tr>
</table>
...
def fill_fields_in_xml(xml_bytes: bytes, values: Dict[str, str]) -> bytes:
    """
    values 예:
      {"SLOT_DATE": "2026-01-16", "SLOT_TITLE": "정기 회의", ...}

    안정적인 주입 원칙
    - 슬롯의 경계는 fieldBegin/fieldEnd로 정의한다.
    - fieldBegin 이후에 나타나는 "가장 가까운" fieldEnd(beginIDRef=id)가 그 슬롯의 끝.
    - fieldBegin ~ fieldEnd 범위 안에 있는 hp:t들 중:
        * 첫 hp:t: 실제 값으로 치환
        * 나머지 hp:t: 빈 문자열로 (서식/런 구조를 최대한 유지하기 위함)
    """
    parser = etree.XMLParser(recover=True, remove_blank_text=False)
    root = etree.fromstring(xml_bytes, parser=parser)
    ...
    for fb in root.xpath(".//hp:fieldBegin[@name][@id]", namespaces=NS):
        ...
        # fieldBegin 뒤에 나오는 해당 field의 종료 지점(fieldEnd)을 1개로 확정
        ends = fb.xpath(
        ...
        if not ends:
            # 템플릿이 비정상(끝 태그 누락 등)일 수 있음
            continue
        end_node = ends[0]

        # 범위 제한 핵심:
        #  - fb 이후의 hp:t들 중에서
        #  - "다음 fieldEnd(beginIDRef=_toggle)의 첫 번째가 end_node인 것"만 가져오기
        # 이렇게 하면 다른 슬롯의 텍스트까지 침범하지 않습니다.
        t_nodes = fb.xpath(
        ...
        if t_nodes:
            ...
            # 범위 안에 hp:t가 없다면, end_node 직전에 hp:t를 삽입해 값이 들어가게 만듭니다.
            # (문서 파손 방지를 위해 최소한의 삽입만 수행)
            ctrl = end_node.getparent()  # 보통 hp:ctrl
            if ctrl is not None:
...
<table>
  <tr>
    <td><img src="..\images\image-20260116213845536.png" width="420"></td>
    <td><img src="..\images\image-20260116213931240.png" width="420"></td>
  </tr>
</table>
